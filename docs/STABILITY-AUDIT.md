...
| S-4 | ✅ DONE (v2.11.0 screen renders, v2.11.1 insights). Single requestRender(reason,{immediate,target}) scheduler — both render targets (screen + insights) funnel through it with per-target debounce timers; coalesced by default, immediate for direct user actions. Pure timing logic in src/render-scheduler.js (tested incl. multi-target independence). Unblocks T-DND-1. 🔶 PHASE 1 DONE (v2.11.0) Single render scheduler — screen renders funnel through requestRender(reason, {immediate}); coalesced by default, immediate for direct user actions. Pure timing logic in src/render-scheduler.js (tested). PHASE 2 PENDING: fold the ~8 renderInsights() triggers in too. Centralize rendering: one scheduler all 23 triggers go through; move fingerprint/snapshot into it | 3 | M | med | kills the flicker class structurally |
| S-5 | ✅ DONE (v2.11.2 + v2.11.3). Split the 476-line fetchJiraData monolith into named async helpers IN background.js (conservative — stateful async/live-API/TDZ, no module move): fetchCurrentSprint, fetchSprintHistory, fetchSupportTickets, fetchMilestones, fetchExtraBoards (+fetchOneExtraBoard). fetchJiraData is now a 42-line orchestrator of named steps (476→42). TDZ ordering preserved verbatim; no leaked locals; 23 suites + pre-flight green. NOT byte-verifiable (no live Jira here) — load to confirm. 🔶 IN PROGRESS (v2.11.2 batch 1/?: extracted fetchSprintHistory/fetchSupportTickets/fetchMilestones as named helpers IN background.js — conservative, no module move, since this is stateful async w/ TDZ traps + live API that can't be proven byte-identical). fetchJiraData 476→443. REMAINING: the big current-sprint block + extra-boards block. Split `fetchJiraData` into named (pure-where-possible) steps + tests | 3 | M | med | kills the TDZ/ordering class |
| S-6 | ✅ DONE (v2.11.4). Design tokens established v2.8.7 (--fs-caption/label/body/subhead/head, --space-1..4). v2.11.4 migrated all 79 mappable HTML inline font-sizes in popup.js to --fs-* tokens (10→caption,11→label,12→body,13→subhead,16→head). SVG font-sizes N/A (all moved to src/render/ during S-3). 9px×3 + 19px×1 left as off-scale. Design tokens: type scale + spacing + section-header class; migrate inline styles section by section | 1 | L | low | consistent hierarchy + huge maintainability win |
| S-7 | ✅ DONE (v2.8.7 helper + v2.11.4). emptyState(msg,icon,action) helper + .empty-state CSS exist; v2.11.4 migrated 3 ad-hoc inline empty-states (analytics/points/worklog) to the helper. 2 contextual ternary ones left inline (low value, would contort). Consistent empty/loading/error state component | 1 | S | low | "looks broken" → "nothing to show" |
| S-8 | ✅ DONE (v2.10.1 background + v2.11.4 popup). background.js console.errors audited v2.10.1 (6→warn). v2.11.4 audited popup.js: downgraded theme-load + snooze to warn (cosmetic/non-fatal); kept loadData/refresh/gantt-tab/partial-update as error AND surfaced loadData + gantt-tab failures to the user via showErrorBanner (were silent). Audit silent catches; add context logs / `safe()` wrapper | 3 | S | low | no more multi-week hidden failures |
