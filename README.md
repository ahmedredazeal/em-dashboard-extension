...
- **Time Utilization** — optional Google Calendar overlay on the squad time chart: each member's meeting/busy hours appear as a hatched sub-bar beside logged hours (teammate data is busy-only; overlaps merged). When connected it also powers the **Today's Meetings** card with your own real meeting titles, with the iCal URL kept as a cross-browser fallback (the card shows a titled list in Google mode, a generic countdown in iCal mode). Auth via `chrome.identity.getAuthToken` + a Chrome Extension OAuth client (scope `calendar.readonly`, client ID in `manifest.oauth2`, stable ID pinned via `key`); Chrome-only, off by default. Squad emails for the overlay can be auto-filled from Jira (per-member accountId → email) via a button in Settings, with manual fallback. The Settings page groups sections into cards (Integrations / Team / Preferences / Insights & alerts) with footer links; utilization emails auto-fill from Jira once Jira + Google are connected. Full-day/OOO blocks are excluded from meeting hours and shown as a days-off (🌴N) badge, not counted as busy. Rougher sections (Estimate vs Actual, Reopen rate, Monthly report) carry an ALPHA badge.
