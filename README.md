...
- **Time Utilization** — optional Google Calendar free/busy overlay on the squad time chart: each member's meeting/busy hours appear as a hatched sub-bar beside logged hours (busy times only, no meeting titles; overlaps merged). Auth via `chrome.identity.getAuthToken` with a Chrome Extension OAuth client (client ID in `manifest.oauth2`, stable ID pinned via `key`); Chrome-only, off by default.
